...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  input.forEach(item =>{
  let smallArr=item.filter( item2 =>{
  if(item2%5 ==0){
  ...
  newArr.forEach(item3 =>{
   let newArr3=item3.map(item4 =>{
    return Math.pow(2, item4);
...
let maXim= Math.max(...newArr);
let indEx=newArr.indexOf(maXim);
...
let findShortest = (data) => {
    ...
    console.log(newArr);
  let miIn=Math.min(...newArr);
  
  let maIstr = miIn.toString();
 
  
  let indEx=newArr.indexOf(maIstr);
